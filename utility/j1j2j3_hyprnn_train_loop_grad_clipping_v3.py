...
#This code is a minor tweak of the J1J2 code (originally written in TF1) at
...
gcv = 3.0
print(f'Norm clipping by value in the range [{-gcv, gcv}] for hyperbolic networks')
#print('Training with gradient clipping by global norm of 8.0 (both Euclidean and hyperbolic)')
#print('The norm clipping is defined in the Adam optimizer')
...
#---------------------------------------------------------------------------------------
def J1J2J3_local_energies(wf, N,J1,J2,J3,Bz,numsamples, samples, Marshall_sign):
    ...
    log_amplitudes = np.zeros(3*N*numsamples, dtype=np.complex64) 
    ...
    slices, len_sigmas = J1J2J3Slices(J1,J2,J3,Bz,samples, sigmas, H, sigmaH, matrixelements, Marshall_sign)
...
def cost_fn(Eloc, log_amplitudes_):
        ...
        return cost
#----------------------------------------------------------------------------------------------------
def train_step(wf, numsamples, input_dim, J1, J2, J3, Bz, Marshall_sign, optimizer):
        with tf.GradientTape() as tape:
            ...
            N = tsamp.shape[1]
            Eloc = J1J2J3_local_energies(wf,N,J1,J2, J3,Bz,numsamples, tsamp, Marshall_sign)
...
#----------------------------------------------------------------------------------------------------
def train_step_with_hypvars(wf, numsamples, input_dim, J1, J2, J3, Bz, Marshall_sign, opt_eucl, opt_hyp):
        ...
        #print(f'length of grads_eucl is {len(grads_eucl)}')
        grads_hyp = tape.gradient(loss_value, wf_hyp_vars)
        #print(f'length of grads_hyp is {len(grads_hyp)}, {grads_hyp}')
        # Gradient clipping by value
        clipped_grads_eucl = [tf.clip_by_value(grad, -gcv, gcv) for grad in grads_eucl]
        clipped_grads_hyp = [tf.clip_by_value(grad, -gcv, gcv) for grad in grads_hyp]
        opt_eucl.apply_gradients(zip(clipped_grads_eucl, wf_eucl_vars))
        opt_hyp.apply_gradients(clipped_grads_hyp, wf_hyp_vars)
        return loss_value, Eloc, wfmodel


# ---------------- Running VMC with RNNs for J1J2 Model with only Euclidean parameters -------------------------------------
def run_J1J2J3(wf, numsteps, systemsize, var_tol, J1_  = 1.0, J2_ = 0.0, J3_ = 0.0, Marshall_sign = False, 
    ...
    for step in range(numsteps):
        cost, E, wfmodel = train_step(wf, numsamples, input_dim, J1, J2, J3, Bz, Marshall_sign, optimizer)
        meanE = np.mean(E)
        varE = np.var(E)
        ...
        if step !=0:
            if np.real(meanE) < min(np.real(best_E_list)) and  np.abs(varE) < var_tol:
                ...
                wfmodel.save_weights(f'{fname}/N{systemsize}_J1={J1_}|J2={J2_}|J3={J3_}_{wf.name}_ns={numsamples}_Ms{Marshall_sign}_checkpoint.weights.h5') 
...
def run_J1J2J3_hypvars(wf, numsteps, systemsize, var_tol, J1_  = 1.0, J2_ = 0.0, J3_ = 0.0, Marshall_sign = True, 
    ...
    best_E_list = []
    for step in range(numsteps):
        cost, E, wfmodel = train_step_with_hypvars(wf, numsamples, input_dim, J1, J2,J3, Bz, Marshall_sign, opt_eucl, opt_hyp)
        meanE = np.mean(E)
        varE = np.var(E)
        meanEnergy.append(meanE)
        varEnergy.append(varE)
