...
#https://github.com/mhibatallah/RNNWavefunctions/tree/master/2DTFIM_1DRNN/RNNwavefunction.py
...
class RNNwavefunction(object):
    def __init__(self,systemsize_x, systemsize_y,
                   cell_type,  units=10, seed = 111):
        """
            systemsize_x:  int
        ...
        if cell_type == 'EuclRNN':
            ...
            self.name = f'1DRNN_Nx={self.Nx}_Ny={self.Ny}_{cell_type}_{self.units}'
    ...
    def sample(self,numsamples,inputdim):
        ...
        inputs=tf.constant(dtype=self.dtype,value=b,shape=[numsamples,inputdim]) #Feed the table b in tf.
        ...
        samples=[]
        rnn_state=tf.zeros((self.numsamples, self.units),dtype=self.dtype)
        ...
        for ny in range(self.Ny): #Loop over the lattice in a snake shape
            for nx in range(self.Nx):
                ...
                inputs=tf.one_hot(sample_temp,depth=self.outputdim, dtype = self.dtype)
    ...
    def log_probability(self,samples,inputdim):
        ...
        a=tf.zeros(self.numsamples, dtype=self.dtype)
        b=tf.zeros(self.numsamples, dtype=self.dtype)
        ...
        for ny in range(self.Ny):
            for nx in range(self.Nx):
                output, rnn_state = self.model.call(inputs, rnn_state)
                probs.append(output)
                inputs=tf.reshape(tf.one_hot(tf.reshape(tf.slice(samples,begin=[np.int32(0),np.int32(ny*self.Nx+nx)],size=[np.int32(-1),np.int32(1)]),
                                                        shape=[self.numsamples]),depth=self.outputdim,dtype = self.dtype),shape=[self.numsamples,self.inputdim])
...
class RNNwavefunction_hyp(object):
    def __init__(self,systemsize_x, systemsize_y,cell_type, bias_geo, hyp_non_lin, units, seed = 111):
        """
        ...
        if cell_type == 'HypRNN':
            ...
            self.name = f'1DRNN_Nx={self.Nx}_Ny={self.Ny}_{cell_type}_{self.units}_{self.h_bias_geo}_{self.h_non_lin}'
