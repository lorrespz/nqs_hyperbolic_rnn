...
#Code adapted from the following TF1 code
...
def mobius_test_TF():
    ...
    v1 = tf.compat.v1.placeholder(tf.float64, shape=[bs, emb_dim])
    v2 = tf.compat.v1.placeholder(tf.float64, shape=[bs, emb_dim])
    M = tf.compat.v1.placeholder(tf.float64, shape=[emb_dim, 5])
